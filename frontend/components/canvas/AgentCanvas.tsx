...
function pastelFromId(id: string): {
	...
	// Deterministic pastel based on id hash
	let hash = 0;
	for (let i = 0; i < id.length; i++)
		hash = (hash * 31 + id.charCodeAt(i)) >>> 0;
	const hue = hash % 360;
	const bg = `hsl(${hue} 100% 95%)`;
...
export function AgentCanvas({
	...
	const prevNodesRef = useRef<string>(
		JSON.stringify(rfNodes.map((n) => ({ id: n.id, type: n.type })))
	);
	const prevEdgesRef = useRef<string>(
		JSON.stringify(
			rfEdges.map((e) => ({ id: e.id, source: e.source, target: e.target }))
		)
	);
	...
	useEffect(() => {
		...
		const currentStructure = JSON.stringify(
			rfNodes.map((n) => ({ id: n.id, type: n.type }))
		);
			...
			changes.forEach((change) => {
				if (change.type === "position" && change.position) {
					...
					if (!change.dragging) {
						...
						if (
							lastSaved &&
							lastSaved.x === currentPos.x &&
							lastSaved.y === currentPos.y
						) {
							return;
						}
						...
						const timer = setTimeout(() => {
							...
							lastSavedPositionsRef.current.set(nodeId, {
								x: currentPos.x,
								y: currentPos.y,
							});
